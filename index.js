...
function lotsOfArt(array) {
  const newArray = array.filter((artist) => artist.paintings > 100);
  const justNames = newArray.map(({name}) => (name));
  return justNames;
}
...
function artistInfo(array, name){
  return array.filter((artists) => artists.name === name).map(({bio}) => (bio)).toString();
  }
...
function artistByCountry(array,nationality){
  return array.filter((artists) => artists.nationality === nationality).map(({name}) => (name));
  }
